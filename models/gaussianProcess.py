...
class ModelClass():
  ...
  def ModelTraining(self):
    ...
    self.predictorModel = []
    for app in range(0, self.nApp):
      self.predictorModel.append(GaussianProcessRegressor(normalize_y=True).fit(X[app], y[app]))
        ...
        if tile.type != "mem" and tile.type != "emp":
          for app in range(0, self.nApp):
            if tile.type == self.appList[app]:
              ...
              res = self.predictorModel[app].predict(inputConfig.reshape(1, -1), return_std=True)
              mu = res[0][0]
